...
chel_home_pois = [poisson.pmf(i,np.sum(np.multiply(chel_home.values.T,chel_home.index.T),axis=1)[0]) for i in range(6)]
...
chel_away_pois = [poisson.pmf(i,np.sum(np.multiply(chel_away.values.T,chel_away.index.T),axis=1)[0]) for i in range(6)]
...
for i in finalWeek:
    ...
    result[i["Home"]+" vs "+i["Away"]]={"Home Wins":str(int(np.sum(np.tril(match, -1))*100))+"%", "Draws":str(int(np.sum(np.diag(match))*100))+"%","Away Wins":str(int(np.sum(np.triu(match,1))*100))+"%"}
